...
#if defined(NDEBUG)
# error "HALOX cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7dbe2474ac43247586d333c8a11c4c4a97a6b6d12c237f90c7f585be4b618dbb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // HALOX: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "HALOX Signed Message:\n";
...
{
    // HALOX: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // HALOX: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // HALOX: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1491309310, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "HALOX IS BORN";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 40 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046989123c1af41c5f1056274dbbff9ad8ce7f94c5a0db0a3b7e3dd6b4d5499e13e321e7ee10aaf7b41b55bc35ca0ee15e0eefc90078d4053291e7a649a3d11f7c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1491309310;
        ...
        assert(block.hashMerkleRoot == uint256("0x524eda6e1553be6972fff95ab20074bd7105f529eadb04e46bff6cd9d039b940"));
...
unsigned char pchMessageStart[4] = { 0x8d, 0x63, 0xfa, 0x2f }; // HALOX: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HALOXMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HALOXMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HALOXMiner(CWallet *pwallet)
{
    printf("HALOXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("halox-miner");
    ...
    try { loop {
        ...
        printf("Running HALOXMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HALOXMiner, pwallet));
}
